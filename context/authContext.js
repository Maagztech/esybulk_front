...
export const AuthProvider = ({ children }) => {
      ...
      try {
        ...
        if (refresh_token) {
          const response = await axios.post(
            "https://api.esybulk.store/api/refresh_token",
            { refresh_token }
          );
  ...
  const getUserInfo = async (token) => {
    ...
    try {
      const response = await axios.post(
        "https://api.esybulk.store/api/login",
        {
  ...
  const selectRole = async (role) => {
    try {
      const response = await axios.post(
        "https://api.esybulk.store/api/selectRole",
        { role },
        {
  ...
  const activeAccount = async (data) => {
    try {
      const response = await axios.post(
        "https://api.esybulk.store/api/active",
        data,
        { headers: { Authorization: `${access_token}` } }
      );
      setUserInfo(response.data.user);
  ...
  const handleLogout = async () => {
    try {
      axios.get("https://api.esybulk.store/api/logout", {
        headers: { Authorization: `${access_token}` },
      });

...
export const DistributorProvider = ({ children }) => {
  ...
  const fetchPreviousProduct = async () => {
    try {
      ...
      const response = await axios.get(
        `https://api.esybulk.store/api/getHistory`,
        {
  ...
  const fetchProducts = async (page) => {
    try {
      ...
      setLoading(true);
      const response = await axios.get(
        `https://api.esybulk.store/api/distributor/products?page=${page}&query=${query}`,
        {
          headers: { Authorization: `${access_token}` },
        }
      );
  ...
  const handleSearchSubmit = async () => {
    try {
      ...
      setLoading(true);
      const response = await axios.get(
        `https://api.esybulk.store/api/search?search=${searchText}&page=${currentPage}`,
        {
          headers: { Authorization: `${access_token}` },
        }
      );
  ...
  const fetchCart = async () => {
    try {
      const response = await axios.get(
        "https://api.esybulk.store/api/distributor/cart",
        { headers: { Authorization: `${access_token}` } }
      );
  ...
  const addToCart = async (product) => {
    try {
      const response = await axios.post(
        "https://api.esybulk.store/api/distributor/addremovecart",
        { product },
        { headers: { Authorization: `${access_token}` } }
      );
      await fetchCart();
